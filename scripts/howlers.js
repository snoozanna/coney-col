...
var m_map_music = new Howl({
  src: ["src/audio/A3_music.mp3"],
  html5: true,
  loop: true,
});
var m_currPassage = "";
...
var m_map = "";
var m_buttonIsShow = false;
var m_xint1 = setInterval(function () {}, 0);
var m_xint2 = setInterval(function () {}, 0);
var m_xint3 = setInterval(function () {}, 0);

var m_myHowl = m_map_music;
var m_timer = setInterval(function () {}, 0);

setup.MapMusicControl = function (_input) {
  if (_input == 1) {
    m_map_music.play();
  } else if (_input == 0) {
    m_map_music.stop();
  }
  console.log(m_buttonIsShow);
};

setup.CheckPlayTime = function (_sec, _case, _id, _intNum) {
  // var s = _intNum;
  // var u = setInterval(CheckTime,100);
  // m_xint1 = u;
  switch (_intNum) {
    case "m_xint1":
      m_xint1 = setInterval(CheckTime, 100);
      console.log(passage() + "|" + m_xint1);
      break;
    case "m_xint2":
      m_xint2 = setInterval(CheckTime, 100);
      console.log(passage() + "|" + m_xint2);
      break;
    case "m_xint3":
      m_xint3 = setInterval(CheckTime, 100);
      console.log(passage() + "|" + m_xint3);
      break;
  }
  var f = _sec;
  // console.log(passage() + "|" + u + "|"+ window[s]+ "|" + m_xint1);
  function CheckTime() {
    // console.log(m_myHowl.seek() + ">" + f + "|" + passage() + "|" + _intNum);
    if (m_myHowl.seek() > f) {
      switch (_case) {
        case 1:
          //show stuff
          setup.ClearTimers(_intNum);
          console.log(
            passage() +
              " timed out, button is unlocked. " +
              m_myHowl.seek() +
              ">" +
              f,
          );
        ...
        case 3:
          //transition
          setup.GlobalControl(5);
          setup.ClearTimers(_intNum);
          Engine.play(_id);
          break;
        case 4:
          //transition only, no sound stop.
          // clearInterval(u);
          setup.ClearTimers(_intNum);
          Engine.play(_id);
          break;
        case 5:
          ...
          if (id != null) {
            m_buttonIsShow = true;
            document.getElementById(_id).classList.remove("notVis");
            document.getElementById(_id).classList.add("isVis");
          }
          break;
        case 6:
          //hide stuff
          ...
          if (id != null) {
            m_buttonIsShow = true;
            document.getElementById(_id).classList.remove("isVis");
            document.getElementById(_id).classList.add("notVis");
          }
          break;
        default:
          break;
...
setup.PostMapCheck = function (_case, _id) {
  if (m_buttonIsShow == true) {
    console.log(passage() + " Button Unlocked");
    switch (_case) {
      case 1:
        //show stuff
        var id = document.getElementById(_id);
...
setup.GlobalSetup = function (_name) {
  m_buttonIsShow = false;
  console.log(_name + " has loaded.");
  m_myHowl = new Howl({
    src: ["src/audio/" + _name + ".mp3"],
    html5: true,
  });
  if (!_name.includes("music")) {
    m_myHowl.on("end", function () {
      console.log(_name + " has ended.");
      m_isPlaying = false;
      State.variables.isPause = 1;
...
setup.GlobalControl = function (_input) {
  if (_input == 1 && !m_isPlaying) {
    m_myHowl.play();
    UpdateElement("play");
    State.variables.isPause = 0;
    m_isPlaying = true;
    //    WaitForStream();
  } else if (_input == 2) {
    m_myHowl.pause();
    ...
    m_myHowl.stop();
    setup.ClearTimers("all");
    m_isPlaying = false;
    UpdateElement("pause");
    State.variables.isPause = 1;
  } else if (_input == 3) {
    var currtime = m_myHowl.seek();
    if (currtime < 10) {
    }
    if (currtime - 10 > 0) {
      currtime -= 10;
      ...
      if (m_savedPassage != "") {
        ...
        State.variables.isPause = 1;
        m_myHowl.unload();
        Engine.play(m_savedPassage);
      } else {
        currtime = 0;
      }
    }
    //console.log(myHowl.seek() + "||" + currtime);
    m_myHowl.seek(currtime);
    if (!m_isPlaying) {
      m_myHowl.play();
      m_isPlaying = true;
      State.variables.isPause = 0;
    ...
    var currtime = m_myHowl.seek();
    if (currtime + 10 < m_myHowl.duration()) {
      currtime += 10;
      //console.log('hi ' + currtime);
      m_myHowl.seek(currtime);
      if (!m_isPlaying) {
        m_myHowl.play();
        m_isPlaying = true;
  ...
  function WaitForStream() {
    m_timer = setTimeout(OptionOne, 500);
    function OptionOne() {
      m_myHowl.play();
      UpdateElement("play");
      State.variables.isPause = 0;
...
setup.ClearTimers = function (_x) {
  switch (_x) {
    case "all":
      clearInterval(m_xint1);
      clearInterval(m_xint2);
      clearInterval(m_xint3);
      console.log("all cleared");
      break;
    case "m_xint1":
      clearInterval(m_xint1);
      console.log(m_xint1 + "| m_xint1 cleared");
      break;
    case "m_xint2":
      clearInterval(m_xint2);
      console.log(m_xint2 + "| m_xint2 cleared");
      break;
    case "m_xint3":
      clearInterval(m_xint3);
      console.log(m_xint3 + "| m_xint3 cleared");
      break;
...
setup.SavePassage = function (_s) {
  m_savedPassage = _s;
  setup.DoSave();
  //console.log(savedPassage);
};

setup.SetCurrPass = function () {
  m_currPassage = passage();
};

setup.ToCurrPass = function () {
  Engine.play(m_currPassage);
};

setup.ClearPassage = function () {
  m_savedPassage = "";
};
setup.AutoUnlock = function () {
  Howler.autoUnlock = true;
};

setup.SetMap = function (_i) {
  m_map = _i;
};

setup.ShowMap = function () {
  //console.log(map);
  document.getElementById("mapz").src = "src/images/maps/" + m_map + ".png";
};

setup.InitSetup = function () {
  Config.saves.autoload = false;
  var save = Save.slots.get(0);
  if (save != null) {
...
setup.DoSave = function () {
  Save.slots.save(0, passage());
};

setup.LoadSave = function () {
  var save = Save.slots.get(0);
  console.log(save);
  if (save != null) {
    //Save.slots.load(0);
    Engine.play(save.title);
  } else {
    console.log("No save file");
    var cont = document.getElementById("cont");
    if (cont != null) {
      cont.classList.remove("notVis");
      cont.classList.add("isVis");
    }
  }
};

setup.ResetSave = function () {
  Save.clear();
};
